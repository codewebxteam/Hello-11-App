...
export default function ChatScreen() {
    ...
    const driverName = (params.driverName || params.name) as string || "Driver";

    console.log("[DEBUG] Client Chat Screen - bookingId:", bookingId, "Params:", params);
    ...
    useEffect(() => {
        ...
        const handleReceiveMessage = (msg: Message) => {
            console.log("Client received chat msg:", msg);
            setMessages((prev) => {
                // Prevent duplicates
        ...
        const setupSocket = async () => {
            try {
                ...
                console.log("Client joined chat room via init:", bookingId);
            } catch (err) {
                console.error("Socket setup failed in client chat:", err);
            }
        };
        ...
        const fetchHistory = async () => {
            try {
                const res = await chatAPI.getHistory(bookingId);
        ...
        >
            <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }} edges={['top', 'bottom']}>
                ...
                <View style={{ paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F1F5F9', flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff' }}>
                    ...
                    <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 18, fontWeight: '900', color: '#0F172A' }}>{driverName}</Text>
                        <Text style={{ fontSize: 12, fontWeight: '700', color: '#22C55E' }}>● Online</Text>
                    </View>
                </View>
