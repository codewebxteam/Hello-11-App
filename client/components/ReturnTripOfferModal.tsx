    ...
    <Modal transparent visible={isVisible} animationType="none" onRequestClose={isAccepting ? undefined : handleClosePress}>
      <View className="flex-1 bg-black/70 justify-center items-center" style={{ paddingHorizontal: isSmallPhone ? 12 : 16 }}>
        <Animated.View
          ...
          style={{ maxWidth: isTablet ? 560 : 460 }}
        >
          <View className="absolute top-0 left-0 right-0 h-32 bg-[#FFD700] opacity-20" />

          <View className={`items-center ${isSmallPhone ? "pt-5 pb-3" : "pt-8 pb-4"}`}>
            <View className="bg-[#FFD700] p-4 rounded-full mb-3 shadow-lg shadow-orange-200">
              <Ionicons name="repeat" size={32} color="black" />
            </View>
            <Text className={`${isSmallPhone ? "text-xl" : "text-2xl"} font-black text-slate-900 text-center px-4`}>
              Book Return Trip?
            </Text>
            <Text className="text-slate-500 font-bold text-center mt-1">Driver will wait for you</Text>
          </View>
          ...
          <View className="mx-5 bg-slate-50 border border-slate-100 p-4 rounded-2xl mb-6 relative overflow-hidden">
            ...
            <View className="flex-row items-end justify-center mb-2">
              <Text className={`${isSmallPhone ? "text-4xl" : "text-5xl"} font-black text-slate-900`}>50</Text>
            ...
            <View className="mt-4 pt-4 border-t border-slate-200">
              ...
              <View className="flex-row items-center">
                ...
                <Text className="text-slate-500 text-xs font-bold ml-2 flex-1">Pay Rs 100/hour waiting fee after that</Text>
              </View>
            </View>
          </View>
